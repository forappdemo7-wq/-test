...
<<<<<<< HEAD
    ...
    // 18. Create Saved Audio Tracks Table
    await query(`
    ...
    // Clean up any old mock user rows or test accounts with hardcoded IDs
    await query(`
      DELETE FROM users 
      WHERE id IN ('user_current', 'user_marcus', 'user_luna', 'user_sofia', 'user_kai', 'user_amara', 'user_liam', 'user_chloe')
