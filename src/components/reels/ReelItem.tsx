      ...
      <div className="absolute inset-x-0 bottom-0 z-20 p-4 pb-6 flex items-end justify-between gap-3">
            ...
            onClick={(e) => {
              ...
              openAudioDetail(
                reel.musicTrack || {
                  ...
                  artist: reel.author?.username || 'creator',
                }
              );
            }}
            className="flex items-center gap-2 text-xs text-white/90 hover:text-white font-medium bg-black/30 hover:bg-black/50 px-2.5 py-1 rounded-full backdrop-blur-sm transition-colors cursor-pointer select-none max-w-fit"
            title="Open Audio Hub"
          >
            <div className="p-0.5 rounded-full">
              <Music size={12} className={isPlaying ? 'animate-bounce text-pink-400' : ''} />
            </div>
        ...
        <div className="flex flex-col items-center gap-3.5 text-white">
          ...
          <button
            ...
            className={`w-9 h-9 rounded-full bg-gradient-to-tr from-neutral-900 to-neutral-700 border-2 border-white/80 flex items-center justify-center shadow-soft-xs cursor-pointer hover:scale-110 transition-transform ${
