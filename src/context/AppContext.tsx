...
interface AppContextType {
  ...
  deleteUserNote: () => Promise<void>;
  sendNoteReply: (targetUser: User, replyText: string) => Promise<void>;
  ...
  openAudioDetail: (
    track:
      | AudioTrack
      | { title: string; artist: string; id?: string; audioUrl?: string; coverUrl?: string }
  ) => void;
  closeAudioDetail: () => void;
  preselectedAudioTrack: AudioTrack | null;
...
export const AppProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const [savedAccounts, setSavedAccounts] = useState<User[]>(() => {
    ...
    const single = safeGetJSON<User | null>(STORAGE_KEYS.AUTH_USER, null);
    if (single && single.id && single.id !== 'guest_user' && !single.username?.toLowerCase().includes('demo')) {
  ...
  const [preselectedAudioTrack, setPreselectedAudioTrack] = useState<AudioTrack | null>(null);
  ...
  const [blockedUserIds, setBlockedUserIds] = useState<string[]>(() => {
    const u = safeGetJSON<User | null>(STORAGE_KEYS.AUTH_USER, null);
    if (u && u.id && u.id !== 'guest_user') {
      const savedBlocked = safeGetJSON<string[] | null>(`${STORAGE_KEYS.BLOCKED_USERS_PREFIX}${u.id}`, null);
  ...
  const refreshData = useCallback(async () => {
    try {
      ...
      try {
        ...
        if (notesRes.ok) {
          const notesData = await safeJson(notesRes);
          if (Array.isArray(notesData)) {
            setCommunityNotes(notesData);
            setAvailableProfiles((prev) =>
              prev.map((profile) => {
                const found = notesData.find((n: any) => n.userId === profile.id);
                if (found) {
                  return {
                    ...profile,
                    note: {
  ...
  const publishUserNote = async (
    text: string,
    emoji: string = '💭',
    musicTrack: AudioTrack | null = null,
    audience: 'followers_you_follow_back' | 'close_friends' = 'followers_you_follow_back'
  ) => {
    if (!currentUser?.id) return;
    ...
    // 1. Optimistic update on current user
    const updatedUser = { ...currentUser, note: updatedNote };
    setCurrentUser(updatedUser);
    safeSetJSON(STORAGE_KEYS.AUTH_USER, updatedUser);

    // 2. Update threads
    setThreads((prev) =>
      prev.map((t) => (t.participant.id === currentUser.id ? { ...t, note: updatedNote } : t))
    );

    // 3. Update communityNotes list
    setCommunityNotes((prev) => {
      const filtered = prev.filter((n) => n.userId !== currentUser.id);
      return [
        {
          userId: currentUser.id,
          text: trimmedText,
    ...
    try {
      ...
      console.warn('Failed to persist note to backend:', e);
    }

    if (currentUser.id && currentUser.id !== 'guest_user') {
      updateUserNoteInFirestore(currentUser.id, trimmedText, emoji);
    }
  ...
  const deleteUserNote = async () => {
    if (!currentUser?.id) return;
    const updatedUser = { ...currentUser, note: undefined };
    setCurrentUser(updatedUser);
    safeSetJSON(STORAGE_KEYS.AUTH_USER, updatedUser);

    setThreads((prev) =>
      prev.map((t) => (t.participant.id === currentUser.id ? { ...t, note: undefined } : t))
    );

    setCommunityNotes((prev) => prev.filter((n) => n.userId !== currentUser.id));

    try {
      await fetch(`/api/notes?userId=${currentUser.id}`, { method: 'DELETE' });
    } catch (e) {
      console.warn('Failed to delete note in backend:', e);
    }
    toast.success('Note deleted');
  };

  const sendNoteReply = async (targetUser: User, replyText: string) => {
    if (!currentUser?.id || !targetUser?.id) return;
    const note = targetUser.note;
    const threadId = getDeterministicChatId(currentUser.id, targetUser.id);

    // Ensure thread and chat view open
    ...
    await sendMessage(
      ...
      undefined,
      {
        ...
        noteContext: {
          ...
          emoji: note?.emoji,
          musicTrackTitle:
            typeof note?.musicTrack === 'object' && note?.musicTrack?.title
  ...
  const openAudioDetail = (
    ...
    let fullTrack: AudioTrack | undefined;
    if ('id' in track && track.id) {
      fullTrack = POPULAR_SOUNDTRACKS.find((t) => t.id === track.id);
    }
    if (!fullTrack) {
      fullTrack = POPULAR_SOUNDTRACKS.find(
        (t) => t.title.toLowerCase() === track.title.toLowerCase()
      );
    }
    if (!fullTrack) {
      fullTrack = {
        id: ('id' in track && track.id) ? track.id : `track_${Date.now()}`,
        title: track.title,
        artist: track.artist || 'Original Audio',
        audioUrl:
          track.audioUrl ||
  ...
  const openCreateWithAudio = (track: AudioTrack, createType: 'post' | 'reel' = 'reel') => {
    setPreselectedAudioTrack(track);
