...
var UserRepository = class extends BaseRepository {
  ...
  async getPendingFollowRequests(targetUserId) {
    const res = await query(
      `SELECT 
  ...
  async acceptFollowRequest(requesterId, targetUserId) {
    await query("DELETE FROM follow_requests WHERE requester_id = $1 AND target_id = $2", [
      requesterId,
      targetUserId
    ]);
  ...
  async acceptAllFollowRequests(targetUserId) {
    await query(
  ...
  async setCloseFriends(userId, friendIds) {
  ...
  async isRestricted(userId, targetId) {
    const res = await query(
...
var UserService = class {
  ...
  async declineFollowRequest(currentUserId, requesterId) {
    if (!currentUserId || !requesterId) {
      throw new BadRequestError("Current user and requester IDs are required");
    }
  ...
  async getCloseFriends(currentUserId) {
    if (!currentUserId) throw new BadRequestError("User ID is required");
    return userRepository.getCloseFriends(currentUserId);
  }
  async toggleCloseFriend(currentUserId, friendId) {
  ...
  async setCloseFriends(currentUserId, friendIds) {
    if (!currentUserId) throw new BadRequestError("User ID is required");
    await userRepository.setCloseFriends(currentUserId, friendIds || []);
    return { success: true };
  }
  async getRestrictedUsers(currentUserId) {
    if (!currentUserId) throw new BadRequestError("User ID is required");
    return userRepository.getRestrictedUsers(currentUserId);
  }
  async restrictUser(currentUserId, targetUserId) {
  ...
  async unrestrictUser(currentUserId, targetUserId) {
    if (!currentUserId || !targetUserId) throw new BadRequestError("User IDs required");
...
var UserController = class {
  ...
  async getCloseFriends(req, res, next) {
    try {
      const currentUserId = req.query.currentUserId || req.user?.id || req.params.id;
  ...
  async toggleCloseFriend(req, res, next) {
    try {
  ...
  async setCloseFriends(req, res, next) {
    try {
      const currentUserId = req.body.currentUserId || req.user?.id || req.params.id;
  ...
  async getRestrictedUsers(req, res, next) {
    try {
      const currentUserId = req.query.currentUserId || req.user?.id || req.params.id;
  ...
  async unrestrictUser(req, res, next) {
    try {
      const currentUserId = req.body.currentUserId || req.user?.id;
      const targetUserId = req.params.id || req.body.targetUserId;
...
router2.get("/:id/close-friends", optionalAuth, userController.getCloseFriends.bind(userController));
router2.post("/:id/close-friends", optionalAuth, userController.setCloseFriends.bind(userController));
router2.post("/:id/close-friends/:friendId/toggle", optionalAuth, userController.toggleCloseFriend.bind(userController));
...
var StoryRepository = class extends BaseRepository {
  ...
  async votePoll(storyId, userId, optionId) {
    ...
    poll.options = poll.options.map((opt) => {
      const voters = Array.isArray(opt.voterUserIds) ? opt.voterUserIds.filter((id) => id !== userId) : [];
      if (opt.id === optionId) {
        voters.push(userId);
      }
      return {
    ...
    const totalVotes = poll.options.reduce((acc, o) => acc + (o.votesCount || 0), 0);
    poll.totalVotes = totalVotes;
    poll.userVotedOptionId = optionId;
    await query(`UPDATE stories SET poll = $1 WHERE id = $2`, [JSON.stringify(poll), storyId]);
    return poll;
  }
  async submitQuestionResponse(storyId, user, responseText) {
...
var StoryService = class {
    ...
    for (const s of rawStories) {
      ...
      if (poll && Array.isArray(poll.options)) {
        const votedOption = poll.options.find(
          (opt) => Array.isArray(opt.voterUserIds) && opt.voterUserIds.includes(currentUserId)
        );
  ...
  async submitQuestionResponse(storyId, user, response) {
...
var StoryController = class {
  ...
  async submitQuestionResponse(req, res, next) {
    try {
      const { id } = req.params;
...
var CreateStorySchema = z4.object({
  ...
  question: z4.any().optional(),
  music: z4.any().optional()
});
var VotePollSchema = z4.object({
  userId: z4.string().optional(),
  optionId: z4.string().min(1, "Option ID is required")
});
var SubmitQuestionSchema = z4.object({
  userId: z4.string().optional(),
  ...
  async markChatSeen(chatId, currentUserId) {
    if (chatId && chatId.startsWith("chat_")) {
      ...
      if (rest.startsWith(currentUserId + "_")) {
        otherUserId = rest.slice(currentUserId.length + 1);
      } else if (rest.endsWith("_" + currentUserId)) {
...
router10.get("/", optionalAuth, async (req, res, next) => {
  try {
    const currentUserId = req.query.currentUserId || req.user?.id || "";
    let result;
    try {
      ...
      if (queryErr?.message?.includes("does not exist") || queryErr?.message?.includes('relation "user_notes"')) {
        await query(`
          CREATE TABLE IF NOT EXISTS user_notes (
            ...
            emoji VARCHAR(16) DEFAULT '\u{1F4AD}',
...
router10.post("/", optionalAuth, async (req, res, next) => {
  try {
    ...
    const { text, emoji = "\u{1F4AD}", musicTrack = null, audience = "followers_you_follow_back" } = req.body;
    if (!text || !text.trim()) {
...
router10.delete("/", optionalAuth, async (req, res, next) => {
  try {
...
var noteRoutes = router10;

// server/routes/v1/audio.routes.ts
import { Router as Router11 } from "express";
var router11 = Router11();
var DEFAULT_SOUNDTRACKS = [
  {
    id: "track_1",
    title: "Espresso",
...
router11.get("/", optionalAuth, async (req, res, next) => {
  try {
    const currentUserId = req.query.currentUserId || req.user?.id || "";
    let savedTrackIds = [];
    if (currentUserId) {
...
router11.get("/:id", optionalAuth, async (req, res, next) => {
  try {
    const { id } = req.params;
    const currentUserId = req.query.currentUserId || req.user?.id || "";
...
var handleToggleSave = async (req, res, next) => {
  try {
    const userId = req.body.userId || req.user?.id;
...
router11.post("/save", optionalAuth, handleToggleSave);
...
router12.use("/posts", postRoutes);
router12.use("/stories", storyRoutes);
router12.use("/reels", reelRoutes);
router12.use("/messages", messageRoutes);
router12.use("/notifications", notificationRoutes);
router12.use("/highlights", highlightRoutes);
router12.use("/gemini", aiRoutes);
router12.use("/notes", noteRoutes);
router12.use("/audio", audioRoutes);
